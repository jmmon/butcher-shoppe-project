function ServicesButchering({ bg }) {
	return (
		<div className="card-wrapper" id="butchering-link">
			<div className="panel--shadow card">
				<div
					className="container inset-box-shadow card"
					style={{
						background: `url(${bg}) center center/cover no-repeat`,
					}}
				>
					<h2 className="card-heading card-heading-shadow">
						Farm Kill
					</h2>
					<div className="card-content-wrapper">
						<div className="skinny-blerb text-wrapper-shadow">
							<p>
								On the scheduled day of your appointment, you
								will be asked to safely secure your animal(s)
								where dispatch can occur without harming other
								livestock. The designated animal(s) will be
								killed, hung, eviscerated, de-hided, de-headed
								and de-legged on sight.
							</p>
							<br />
							<p>
								You may choose to retain any or all of the
								contents removed from the animal. We will
								dispose of the rest. Then your animal(s) will be
								weighed and loaded into a refrigerated box and
								transported to the designated shop. Kill and
								leave is an option as well.
							</p>
						</div>
						{/* <div className="butchering-grid">
							<div className="butchering-grid-title">Animal</div>
							<div className="butchering-grid-title">Price</div>
							<div className="butchering-grid-data">Cow</div>
							<div className="butchering-grid-data">Cow Price</div>
							<div className="butchering-grid-data">Pig</div>
							<div className="butchering-grid-data">Pig Price</div>
							<div className="butchering-grid-data">Goat</div>
							<div className="butchering-grid-data">Goat Price</div>
						</div> */}

						{/* <table className="butchering">
							<tr className="butchering-row">
								<th>Animal</th>
								<th>Price</th>
							</tr>
							<tr className="cow">
								<td className="animal">Cow</td>
								<td className="price">$209</td>
							</tr>
							<tr className="pig">
								<td className="animal">Pig</td>
								<td className="price">$189</td>
							</tr>
							<tr className="goat">
								<td className="animal">Goat</td>
								<td className="price">$169</td>
							</tr>
						</table> */}
					</div>
				</div>
			</div>
		</div>
	);
}

export default ServicesButchering;
