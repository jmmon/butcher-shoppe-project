...
const Contact = () => {	
	...
	const handleSubmit = (e) => {
		...
		setSentToUs(prevState => ({...prevState, status: "PENDING", errMsg: null}));

		// send us the email
		emailjs.sendForm(emailJs.service_toUs, 'contact_form', form.current, emailJs.userId)
			.then(() => {
				setSentToUs(prevState => ({...prevState, status: "COMPLETE"}));

				console.log('timer set');
				timer = setTimeout(() => { // reset button timer
					console.log('timer ended');
					setSentToUs(prevState => ({...prevState, status: "WAITING"}));
				}, 4000);
			
				setInput(prevInput => { // clear input boxes
					return {
						...prevInput,
						contact__name: '',
						contact__email: '',
			...
			.catch((err) => {
				...
				setSentToUs(prevState => ({...prevState, status: "ERROR", errMsg: err}));
			});
		...
		<div className="contact">
			...
			<form className="contact__form" ref={form} onSubmit={handleSubmit}>
				...
				<div className="input__container input__large">
					...
					<textarea 
						type="text" 
						className="contact__input contact__textarea footer-input" name="contact__textarea" 
				...
				{(sentToUs.status === 'WAITING' || sentToUs.status === "ERROR") && <button 
					type="submit" 
				...
				{sentToUs.status === 'PENDING' && <button 
					type="submit" 
				...
				{sentToUs.status === 'COMPLETE' && <button 
					type="submit" 
				...
				{sentToUs.errMsg && <p className="contact__form__err">{sentToUs.errMsg}</p>}
