...
function Tabs({tabs = []}) {
	const inputs = tabs.map((tabData, index) => <input type="radio" id={`react-tab${index+1}`} name="css-tabs-react" />);

	const labels = tabs.map((tabData, index) => <li className={styles.label}><label htmlFor={`react-tab${index+1}`}>{tabData.title}</label></li>);
