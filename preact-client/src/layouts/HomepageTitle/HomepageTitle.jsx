...
export default function HomepageTitle({
  ...
  return (
    <section
      className={`${Styles.section} text-center text-white panel-shadow--dark pane`}
    >
      <Fade {...properties}>
      ...
      <div className={`flex-col-jaround-acenter ${Styles.content_wrapper}`}>
        ...
        <i className={`${Styles.subtext} flex text-shadow--lg`}>
          <span className={`${Styles.first}`}>{firstSubtext + ' '}</span>
          <span className={`${Styles.second}`}>{secondSubtext}</span>
        </i>
      </div>
    </section>
  );
